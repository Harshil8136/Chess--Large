    ...
    const GameReviewer = {
        ...
        analyze: async function(pgn, stockfish, progressCallback) {
            ...
            for (let i = 0; i < history.length; i++) {
                ...
                Logger.debug(`Analyzing position for move ${i + 1}`, { fen: tempGame.fen() });
                let positionEval;
                try {
                    positionEval = await this._getEvaluationWithRetry(stockfish, tempGame.fen());
                } catch (error) {
                    Logger.warn(error.message, error);
                    positionEval = { best: lastEval, second: lastEval, best_pv: '' };
                }
        ...
        /**
         * NEW: An async wrapper that attempts an evaluation and retries once on failure.
         */
        _getEvaluationWithRetry: async function(stockfish, fen) {
            try {
                return await this._singleEvaluationAttempt(stockfish, fen, REVIEW_ENGINE_OPTIONS.movetime);
            } catch (error) {
                Logger.warn(`Evaluation timed out. Retrying with longer time...`, new Error(error.message));
                // On the second attempt, if it fails again, the error will propagate up and be caught by the main loop.
                return await this._singleEvaluationAttempt(stockfish, fen, RETRY_MOVETIME);
            }
        },

        /**
         * NEW: Performs a single evaluation attempt with a watchdog timer.
         */
