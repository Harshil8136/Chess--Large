...
function initLogBox() {
    ...
    // UPDATED: Correctly defined selector for the new stats drawer
    ...
    // --- UPDATED: Advanced Features Logic ---
    async function updateStatsDrawer() {
        const session = Logger.getLogs(Logger.getCurrentSessionId());
        $('#log-fps-panel').html(`<span><b>FPS:</b> ${currentFPS}</span>`);
        if (performance.memory) {
            const mem = performance.memory;
            $('#log-memory-panel').html(`<span><b>Heap:</b> ${(mem.usedJSHeapSize / 1048576).toFixed(1)}MB</span>`);
        }
        if (session) {
             const duration = Math.floor((new Date() - new Date(session.startTime)) / 1000);
            $('#log-session-panel').html(`<span><b>Logs:</b> ${session.logs.length} | <b>Up:</b> ${duration}s</span>`);
        }
        if (ADVANCED_FEATURES_CONFIG.enableGitHubIntegration && !$('#log-github-panel').data('loaded')) {
            if (window.location.hostname.endsWith('github.io')) {
                try {
                    ...
                    $('#log-github-panel').html(`<a href="${data.html_url}" target="_blank" class="flex items-center gap-2"><span><b>${data.stargazers_count}</b> Stars</span></a>`);
                } catch (error) { Logger.warn('Failed to fetch GitHub repo info', { e: error.message }); }
            }
    ...
    (function initialize() {
        ...
        // UPDATED: Fixed filter dropdown toggle
        controls.filterToggle.on('click', (e) => { e.stopPropagation(); playSound('uiToggle'); controls.filterMenu.toggleClass('visible'); });
        ...
        controls.statsToggle.on('click', () => { playSound('uiToggle'); statsDrawer.toggleClass('is-open'); });
        ...
        // UPDATED: Fixed widget restore bug by stopping event propagation
        controls.widgetRestore.on('dblclick', (e) => { e.stopPropagation(); container.removeClass('widget-mode'); playSound('uiToggle'); });
        
        $(document).on('click', (e) => { if (!controls.filterMenu.is(e.target) && controls.filterMenu.has(e.target).length === 0 && !controls.filterToggle.is(e.target)) controls.filterMenu.removeClass('visible'); });
        $(document).on('newLogEntry', (event, logEntry) => { if (container.is(':hidden')) return; if (logState.visibleLevels[logEntry.level]) { renderVisibleLogs(); if (logState.isAutoScroll) content.scrollTop(9999999); } });
        content.on('scroll', () => { if (scrollTimeout) clearTimeout(scrollTimeout); scrollTimeout = setTimeout(renderVisibleLogs, 50); });
