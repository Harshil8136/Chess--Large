// ===================================================================================
//  MODALS.JS
//  Manages all pop-up modal definitions for the application.
// ===================================================================================

function showPromotionDialog(color, onPromotionSelect) {
...
function showTimeControlModal() {
    const timeOptions = Object.keys(TIME_CONTROLS).map(key => {
        ...
        return `<button class="time-control-btn w-full px-4 py-3 font-bold rounded-lg shadow-md btn-secondary" data-key="${key}">${tc.label}</button>`;
    }).join('');
    Swal.fire({
