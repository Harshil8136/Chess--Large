...
function initApp() {
    ...
    undoButton.on('click', () => {
        ...
        if (window.moveAnalysisData && moveAnalysisData.length >= 2) {
            moveAnalysisData.length -= 2;
        }
    ...
    // UPDATED: Reverted to the fetch() method which is required for online URLs.
    fetch(APP_CONFIG.STOCKFISH_URL)
        .then(response => { 
        ...
        .then(text => {
            try {
                ...
                // --- LIVE ANALYSIS ENGINE (NON-CRITICAL) ---
                ...
                throw workerError; // Re-throw to be caught by the outer .catch()
            }
            
            // This code will now run even if the live analysis engine fails to start.
